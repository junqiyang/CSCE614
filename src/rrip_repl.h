...
struct TreeNode {
    uint64_t index; //index for the tree node
    uint64_t plru_bit;
    bool is_left;
    TreeNode *parent; 
...
class PLRUReplPolicy : public ReplPolicy{
    ...
    public:
        explicit PLRUReplPolicy(uint32_t _numLines, uint32_t _ways)  {
            ways = _ways;
            numLines =_numLines;
            root_array = gm_calloc<TreeNode*>(ways);
            leaf_size = numLines / ways;            
        ...
        template <typename C> inline uint32_t rank(const MemReq* req, C cands) {
            ...
            for(uint32_t i=0;i<leaf_size*ways;i++){
